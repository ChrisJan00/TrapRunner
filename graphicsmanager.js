var GraphicsManager = function() {
	...
	self.clearAll = function(_fillStyle) {
		for (var ii=0; ii < self.layers.length; ii++)
			self.getContext(self.layers[ii]).clearRect(0, 0, self.width, self.height);
		var bgContext = self.getContext(self.layers[0]);
		bgContext.fillStyle = _fillStyle;
		bgContext.fillRect(0,0,self.width,self.height);
